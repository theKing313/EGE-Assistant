...
export async function getHint({ subject, taskText, level }) {
  ...
  if (cached) {
    console.log(
      `[AI] Cache hit provider=${provider.name} key=${cacheKey.slice(0, 12)}… (used ${cached.used_count}x)`,
    );
  ...
  console.log(
    `[AI] Calling provider=${provider.name} model=${provider.model} subject=${subject} level=${level} key=${cacheKey.slice(0, 12)}…`,
  );
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    ...
    try {
      ...
      console.log(
        `[AI] Success provider=${provider.name} in ${Date.now() - start}ms (attempt ${attempt + 1})`,
      );
  ...
  const error = new Error(
    `${provider.name} API unavailable after ${MAX_RETRIES + 1} attempts: ${lastError?.message || "unknown error"}`,
  );
  error.provider = provider.name;
  error.attempts = MAX_RETRIES + 1;
  error.cause = lastError?.cause || null;
  error.phase = lastError?.phase || null;
  error.statusCode = lastError?.statusCode || null;
  error.responseBody = lastError?.responseBody || null;
