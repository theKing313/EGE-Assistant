...
const entries = [
  {
    id: "stress",
    title: "Орфоэпические нормы — ударение",
  ...
  },
  {
    ...
    taskNumbers: [3],
    keywords: ["площадь", "треугольник", "круг"],
  },
  {
    id: "equations",
    title: "Уравнения",
    keywords: ["уравнение", "дискриминант"],
  },
];
...
test("returns the matched entry and its matched keywords", () => {
  const result = rankMatches(
    entries,
    "В каком слове неверно поставлено ударение?",
  );
  assert.equal(result.match.id, "stress");
  assert.ok(result.matchedKeywords.includes("ударение"));
...
test("uses extra text fragments when matching knowledge", () => {
  const result = rankMatches(entries, "Текст задания", 2, 1.5);
  assert.equal(result.match, null);
});
